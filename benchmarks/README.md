...
# `go-ruby-csv` library-level benchmark harness

Reproducible, cross-runtime benchmark of the **pure-Go `go-ruby-csv` library**
...
reference runtime's own `csv`?*

## Layout

- `go/`          — self-contained Go driver; `go.mod` pins the published library.
- `ruby/csv.rb`  — the equivalent workload; `ruby/_harness.rb` is the shared timer.
